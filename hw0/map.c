...
int recur(int i) {      /* a recursive function */
    ...
    printf("recur call %d: stack@ %p\n", i, (void*)&j); /* fix this so it prints the address of j */
    if (i > 0) {
        return recur(i-1);
    }
    return 0;
...
int main (int argc, char *argv[]) {
    ...
    printf("_main  @ %p\n", (void*)&main); /* fix to print address of main */
    printf("recur @ %p\n", (void*)&recur); /* fix to print address of recur */
    printf("_main stack: %p\n", (void*)&i); /* fix to get address of the stack variable */
    printf("static data: %p\n", (void*)&stuff); /* fix to get address of a static variable */
